...
<xsl:stylesheet 
  ...
  <xsl:param name="chapterid"></xsl:param>
  ...
  <xsl:template match="/TEI/text/body">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$chapterid = 'chapter_all'">
            ...
            <div class="novel-container">
              <xsl:for-each select="/TEI/text/body/div">
                <div class="chapter-div">
                  <!-- use a rails link_to helper with the current chapter-->
                  <xsl:variable name="chapter_url">
                    <xsl:value-of select="./@n"/>
                  </xsl:variable>

                  <h4><xsl:text disable-output-escaping="yes">&lt;</xsl:text>%= link_to "<xsl:value-of select="head[1]"/>", visual_chapter_path({ :chapter =<xsl:text disable-output-escaping="yes">&gt;</xsl:text> "<xsl:value-of select="$chapter_url"/>" }) %<xsl:text disable-output-escaping="yes">&gt;</xsl:text></h4>
      ...
      <xsl:otherwise>
          ...
          <xsl:for-each select="/TEI/text/body/div[@n = $chapterid]">
...
<!--             <div class="panel panel-default">
  ...
  <xsl:template match="head">
    <xsl:choose>
      <xsl:when test="$chapterid = 'chapter_all'"></xsl:when>
